...
- `harnes/` 配下の全ファイル
- `harnes/templates/` の命名規約・参照規約
- 学習台帳の構造（`harnes/learning/`）

## 2) 更新原則

1. **本体とメタを分離する**
   - 実行テンプレートは `harnes/`
   - 改善検討・提案・評価は `harnes_meta/`
2. **番号順で読める構造を維持する**
   - 新規追加時は既存番号体系に従う
3. **1変更1目的**
...
6. 最小運用（Core）で読み順検証
7. Full運用での整合検証
8. 変更サマリとロールバック方針を `harnes_meta/` に記録

## 4) 受け入れ条件（DoD）

- [ ] `harnes/` と `harnes_meta/` が役割分離されている
- [ ] 追加ファイルは番号順で読み順が明確
