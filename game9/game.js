    ...
    createParticleEffect() {
        ...
        const ring = () => {
            ...
            for (let i = 0; i < count; i++) {
                ...
                const particle = this.add.circle(cx, cy, 8, Phaser.Display.Color.RandomRGB().color);
                this.tweens.add({
                    targets: particle,
                    x: x,
                    y: y,
...
class GameScene extends BaseScene {
    ...
    create() {
        ...
        // スコア表示
        const scoreFontSize = calculateResponsiveSize(this, 0.04);
        this.scoreText = this.add.text(width * 0.7, height * 0.05, `スコア: ${this.registry.values.score}`,
            { fontSize: `${scoreFontSize}px`, fill: '#000' }).setOrigin(0, 0);

        // コンボ表示
        this.comboText = this.add.text(width / 2, height * 0.15, '',
            { fontSize: `${scoreFontSize}px`, fill: '#ff1493' }).setOrigin(0.5, 0.5);
    ...
    handleGridClick(row, col) {
        ...
        if (this.gameMode === 'number') {
            ...
            if (clickedCell.number === null) {
                ...
                this.registry.values.score = Math.max(0, this.registry.values.score - 1);
                this.combo = 0;
            ...
            if (clickedCell.number === this.expectedNumber) {
                ...
                this.combo += 1;
                this.scoreText.setText(`スコア: ${this.registry.values.score}`);
                if (this.combo > 1) {
                    this.comboText.setText(`${this.combo}れんぞく！`);
                    this.tweens.add({ targets: this.comboText, alpha: { from: 1, to: 0 }, duration: 800, onComplete: () => this.comboText.setAlpha(1) });
                } else {
