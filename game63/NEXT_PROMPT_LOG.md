...
## LATEST POINTER
- `LATEST_PROMPT_ID: PROMPT-0010`
- `UPDATED_AT: 2026-04-15`
- `NOTE: 次回実行時は PROMPT-0010 を最初に読むこと。`
...
## PROMPT-0010
- Prompt-ID: PROMPT-0010
- Created-At: 2026-04-15
- Status: latest
- Goal:
  - game63版を完成済みとして運用し、以後は保守・微調整のみを安全に実施する。
- Scope:
  - 重大不具合（進行不能・データ保存不全）の修正。
  - 5歳UIの可読性改善（短文ひらがな）に限定した文言調整。
  - QAログに沿った運用確認（必要時のみ）。
- Constraints:
  - game1本体は変更しない。
  - 新規大型機能（新モード・新シーン・新システム）は追加しない。
  - 本質ループ（表示→非表示→再生）を維持する。
- DoD:
  - 1) 修正が game63 配下に限定される。
  - 2) 修正内容と影響範囲が `outputs/` に記録される。
  - 3) LATESTポインタと次回指示が追記で管理される。
- Deliverables:
  - 必要時のみ最小修正コード
  - 保守ログ（`game63/outputs/`）
- Next:
  - 追加要望が来た場合は「保守対応か / 新規開発再開か」を先に判定し、
    新規開発再開時のみ別Prompt-IDを追記する。
